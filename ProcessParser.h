...
string ProcessParser::getProcUser(string pid) {
    ...
    // Use Uid to find username in /etc/password
    stream = Util::getStream("/etc/passwd");
    name =("x:" + result);
    // find user with selected UID
    while (std::getline(stream, line)) {
...
bool ProcessParser::isPidExisting(string pid) {
    DIR* dir;
    bool ret_val = false;
    // check if a dir exists for this 'pid'.  If it exists, then process exists.
    if(!(dir = opendir("/proc")))
        throw std::runtime_error(std::strerror(errno));
